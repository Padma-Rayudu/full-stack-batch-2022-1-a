...
import {getpayments,settlements} from "./expenseslice";
export default function Payments()
{
    ...
    function handlesettlement(exid)
    {
          console.log("expense id...",exid)
          dispatch(settlements(exid));
    }
    ...
    <nav class="nav flex-column">
       ...
       <Link to='/showactivities' className="nav-link active" style={{textDecoration:"none"}}><i class="bi bi-clipboard-check"></i>&nbsp;Activities</Link>
             ...
             <div className=" mx-auto sh" style={{marginTop:"100px"}}> 
             <table className="table table-striped table-hover"style={{"width":"800px"}} >
                 <thead>
                     <tr>
                    ...
                    userpayments&&userpayments.map((p,i)=>{
                         return(
                              <tr>
                                  ...
                                  <td>{p.fullname}</td>
                                  <td>{p.description}</td>
                                  <td style={{textDecoration:p.settlement===true? "line-through":"none"}}>&#x20b9;{p.divamount}</td>
                                  <td><Link to={`/showdetails/${p.id}`}>Show Details</Link></td>
                                  <td><button className="btn btn success" onClick={()=>{handlesettlement(p.id)}} disabled={p.settlement===true?true:false}>Settle Amount</button></td>
                                   {
                                     p.settlement===true?<td style={{color:"green"}}>Payment Done</td>:<td style={{color:"red"}}>Payment Not Done</td>
                                   }
