...
import { singleexpense } from "./expenseslice";
import { Link } from 'react-router-dom';
export default function Showdetails() {
  const dispatch = useDispatch()
  const ss = useSelector(state => state.expenseReducer.single[0])
  console.log("single expensee data..", ss)
  React.useEffect(() => {
    var id = window.location.pathname.split('/')[2];
    dispatch(singleexpense(id))
  }, [])
  ...
  return (
    <div>
      ...
      <div className='d-flex'>
        ...
        <div className='m-3 card p-5  mx-auto sh ' style={{ width: '600px',height:"300px" }}>
          <div className='d-flex'>
            <label className="form-label" htmlFor="new password">Event :</label>&nbsp;&nbsp;
            <label className="form-label" htmlFor="new password"><b>{ss && ss[0]['description']}</b></label>
          </div>
          <div className='d-flex'>
            <label className="form-label" htmlFor="new password">Invested By :</label>&nbsp;&nbsp;
            <label className="form-label" htmlFor="new password"><b>{ss && ss[0]['investedbyname']}</b></label>
          </div>
          <div className='d-flex'>
            <label className="form-label" htmlFor="new password">Amount :</label>&nbsp;&nbsp;
            <label className="form-label" htmlFor="new password"><b>&#x20b9;{ss && ss[0]['amount']}</b></label>
          </div>
          <div className='d-flex'>
            <label className="form-label" htmlFor="new password">Date :</label>&nbsp;&nbsp;
            <label className="form-label" htmlFor="new password"><b>{ss && ss[0]['created']}</b></label>
          </div>
          <div className='d-flex'>
            <label className="form-label" htmlFor="new password">Members :</label>&nbsp;&nbsp;
            <div>
              {
                ss && ss.map((s) => {
                  return (
                    <div> <label className="form-label" htmlFor="new password"><b>{s.memname}</b></label>
                      <label>---&#x20b9;{s.divamount}</label></div>
                  )
                })
              }
            </div>
          ...
          <div>
            <Link to="/myexpenses" style={{ textDecoration: "none" }}><i class="bi bi-arrow-left"></i>Back</Link>
          </div>
        </div>
      </div>
    </div>
